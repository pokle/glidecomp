...
- Run `bun audit`.
- **Read the root `package.json`'s `dependencies` block before you change
  anything** and ask whether each entry has a reason to be there. A stray root
  dependency does not only come from this routine's own `bun update` — the
  2026-09-09 *security review* commit left `hono` there, found a cycle later.
  The legitimate root entries are the ones the root's own code and config use;
  anything a workspace declares for itself does not belong at the root.
- Check for upgrades with `bun outdated` — but **`bun outdated` at the repo root
...
- For each upgrade, note:
  ...
  - **When an advisory names a package you cannot move, read what the fixing release actually changed before accepting the finding as live.** `bun audit` matches on the named package's version string, not on the vulnerable code. The critical AVIF RCE against `astro` (GHSA-26w7-cxv4-gfx2) is fixed only on astro 7.x, which needs the whole Vite 8 move — but astro 7.2.8's changelog entry for it is *"Updates the minimum supported version of Sharp to 0.35.4"*, i.e. the entire fix is a dependency floor this tree's `overrides` already exceeds. Resolve every advisory to the actual vulnerable code in `bun.lock`, in both directions: a package you pinned safe can still be vulnerable through a nested copy (the bullet above), and a package you cannot patch can already be safe through a dependency you do control.
  - **A caret on a `@types/*` package will float it past the runtime it describes.** `@types/react-dom: ^19.2.5` jumped to 19.3.0 the day 19.3.0 shipped, while `react`/`react-dom` stayed pinned at 19.2.8 — bun reports this as `incorrect peer dependency`, and the real cost is that code can typecheck against APIs the installed runtime does not have. Where the runtime is pinned exactly, pin its type packages to the same minor with `~`, and lift runtime and types together.

## 3. Verify locally — before pushing

All seven must pass locally:

```
bun run typecheck:all
bun run --filter '@glidecomp/frontend' typecheck
bun run test:all
bun run build
bun run test:e2e
...
`typecheck:all` reaches the frontend only through the root `tsc` project, so run
the frontend's own `typecheck` too — it is what catches a renamed icon export or
a moved type in a frontend-only dependency.

`bun run build` is the only check that exercises the Astro static build (and its
sharp image pipeline) and the frontend's production chunking. A dependency that
breaks it will otherwise not surface until the Deploy workflow.
