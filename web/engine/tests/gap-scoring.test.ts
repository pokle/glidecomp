...
describe('calculateTimePoints', () => {
  ...
  it('HG: ESS-but-not-goal factor is configurable (local regulations)', () => {
    const half = calculateTimePoints(3600, 3600, false, true, 300, 'HG', 'weighted', 0.5);
    expect(half).toBeCloseTo(150, 1);
    const zero = calculateTimePoints(3600, 3600, false, true, 300, 'HG', 'weighted', 0);
    expect(zero).toBe(0);
  });

  it('HG: no time points without ESS regardless of factor', () => {
    const pts = calculateTimePoints(3600, 3600, false, false, 300, 'HG', 'weighted', 1);
    expect(pts).toBe(0);
  });
});
