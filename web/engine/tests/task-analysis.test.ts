...
describe('task analysis integration (kosci-loop-t1)', () => {
  ...
  it('evaluates registered metrics and renders a report', () => {
    ...
    expect(rendered).toContain('Task Analysis');
  ...
  it('leads the rendered report with the metric separation ranking', () => {
    const rendered = renderTaskAnalysis(evaluateField(field));
  ...
  it('correlates a known-good metric strongly against rank (eval template)', () => {
    ...
    expect(renderTaskAnalysis(report)).toContain('test.flown_distance');
  }, 120_000);
  ...
  it('surfaces a throwing metric as an error without killing the report', () => {
    ...
    expect(renderTaskAnalysis(report)).toContain('ERROR computing this metric: boom');
  }, 120_000);
});
