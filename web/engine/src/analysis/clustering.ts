 ...
 *  - Input is a finished TaskAnalysisReport, so clustering is a pure
 ...
 *    the drop-don't-fill convention everywhere else in task analysis — and
...
export const STYLE_NICKNAMES: Record<string, { high: string; low: string }> = {
  ...
  'decision.km_between_climbs': { high: 'Few-stop racers', low: 'Stop-often flyers' },
...
export function usableMetrics(report: TaskAnalysisReport): MetricReport[] {
 ...
 */
function percentileColumns(report: TaskAnalysisReport, metrics: MetricReport[]): (number | null)[][] {
 ...
 */
export function clusterPilotStyles(report: TaskAnalysisReport): StyleClusterReport | null {
