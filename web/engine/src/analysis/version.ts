...
/**
 * Version of the task-analysis metrics' observable behaviour.
 *
 * The competition API stores it on every materialized `task_analysis`
...
//     over the task analysis; the method text of every metric was rewritten
...
//     analysis/thermal-shape.ts. Purely additive: no metric moves. The
...
// v24: each behavioural metric gains an optional `winning` pair — its label
