 ...
 * lived most of this interface was optional — `setMultiTrack?`, `set3DMode?`
 * and the rest meant "Mapbox does this, Leaflet doesn't". With one provider
 * left, that optionality bought nothing and cost the type checker: a `?.` at
 * every call site can never be false, and renaming a method here turned its
 * callers into silent no-ops instead of compile errors.
 *
 * So every member is required. A future provider that genuinely cannot do
 * something implements it as a no-op rather than omitting it, and the two
 * `supports*` flags stay as the honest way to ask about a capability.
 */
