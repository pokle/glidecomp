/**
 * Comp analysis — the same separation question asked across
 ...
 *
 * Public and SSR'd, same as the task page (loadCompAnalysis +
...
import { VerdictBadge } from "../analysis/SeparationRanking";
import {
  AcrossTasksNote,
  ...
  OutcomeChecksNote,
  VerdictLegend,
} from "../analysis/ReadingNotes";
import { ConsistencyChip } from "../analysis/ConsistencyChip";
import { ConsistencyMap } from "../analysis/charts/ConsistencyMap";
import { MetricGlossary, type GlossaryEntry } from "../analysis/MetricGlossary";
import { underComp, COMP_ANALYSIS_LABEL } from "../lib/crumbs";
...
import type { CompAnalysisLoaderData } from "../loaders";
import {
  ALL_METRICS,
...
export function CompAnalysis() {
  ...
  const initial = useInitialData<CompAnalysisLoaderData>();
  const [data, setData] = useState<CompAnalysisData | null>(
  ...
  const analysisUrl = compId
    ? `/api/comp/${encodeURIComponent(compId)}/analysis`
    : null;
  ...
  useEffect(() => {
    ...
    if (initial && refetchTick === 0) {
      document.title = `GlideComp - Comp analysis: ${initial.analysis.comp_name}`;
      return;
    }
    let cancelled = false;
    (async () => {
      ...
      try {
        ...
        // here (routes/analysis.ts). Worth telling apart, because the 404
        ...
        const body = (await res.json()) as CompAnalysisData;
        setData(body);
        document.title = `GlideComp - Comp analysis: ${body.comp_name}`;
        setStatus("ready");
      } catch {
  ...
  if (status === "notFound") {
    return <NotFound title="Comp analysis not found" />;
  }
        ...
        <Alert className="mt-4">
          ...
          <AlertDescription>
            {status === "error"
              ...
              : "This comp analysis is not available. The competition is possibly not published yet."}
          </AlertDescription>
        </Alert>
      </div>
    );
  ...
  return (
    ...
    // every other page under it (see TaskAnalysis).
    <div className="font-hyperlegible">
      <Breadcrumbs items={crumbs} current={COMP_ANALYSIS_LABEL} />

      <div className="mt-3 min-w-0">
        <h1 className="text-2xl font-bold">{COMP_ANALYSIS_LABEL}</h1>
      ...
      {active && rankedMetrics.length > 0 ? (
        ...
        <Alert className="mt-6">
          ...
          <AlertDescription>
            No task in this competition has been analysed yet. To compute
            one, open a task’s analysis.
          </AlertDescription>
        </Alert>
      )}
    </div>
