/**
 * One task's task-analysis report, fetched once and shared by every page that
 * shows a piece of it.
 *
 * The summary page (pages/TaskAnalysis.tsx) and each of its sub-pages
...
export function useTaskAnalysis(): TaskReportBundle {
  ...
  const initial = useInitialData<TaskAnalysisLoaderData>();
  const [data, setData] = useState<TaskAnalysisData | null>(
  ...
  async function refresh() {
    ...
    try {
      const res = await fetch(
        `/api/comp/${encodeURIComponent(compId)}/task/${encodeURIComponent(taskId)}/analysis/refresh`,
