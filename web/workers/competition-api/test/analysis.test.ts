/**
 * Stale-first task-analysis store + endpoints (migration 0019).
 *
...
import {
  computeAndStoreTaskAnalysis,
  readTaskAnalysisRow,
  revalidateTaskAnalysis,
  FIELD_REVALIDATION_LEASE_MS,
  fieldRowHasResult,
  isFieldRowStale,
  type TaskAnalysisRow,
} from "../src/task-analysis-store";
...
async function seedTask(
  ...
  return {
    ...
    compId,
    taskId,
    path: `/api/comp/${compId}/task/${taskId}/analysis`,
    compPath: `/api/comp/${compId}/analysis`,
    refreshPath: `/api/comp/${compId}/task/${taskId}/analysis/refresh`,
    trackKeys,
    pilotIdNums,
  };
}

async function getRow(taskId: number): Promise<TaskAnalysisRow> {
  const row = await readTaskAnalysisRow(env.DB, taskId);
...
describe("task analysis read path", () => {
  ...
  test("a materialized row serves the report as a HIT with an fa:-prefixed ETag", async () => {
    const t = await seedTask();
    await computeAndStoreTaskAnalysis(env, t.taskIdNum, 0);
  ...
  test("ranks come from the official scores, and totals match them", async () => {
    ...
    // `fa.classes` (task analysis) and `scores.class_scores` (task score) are
  ...
  test("a stale body's ETag differs from the fresh one for the same state key", async () => {
    ...
    await env.DB.prepare(
      `UPDATE task_analysis SET inputs_rev = inputs_rev + 1 WHERE task_id = ?`
    )
      .bind(t.taskIdNum)
      .run();
  ...
  test("an unsupported task stores its reason and serves it instead of retrying forever", async () => {
    ...
    const stored = await computeAndStoreTaskAnalysis(env, t.taskIdNum, 0);
...
describe("task analysis visibility (public; test comps admin-only)", () => {
  test("anyone can read a public (non-test) comp's task analysis", async () => {
    const t = await seedTask();
    await computeAndStoreTaskAnalysis(env, t.taskIdNum, 0);
...
describe("task analysis cache headers", () => {
  ...
  test("a long-stable report gets a long max-age, capped at three months", async () => {
    ...
    await env.DB.prepare(
      `UPDATE task_analysis SET computed_at = ? WHERE task_id = ?`
    )
...
describe("task analysis revalidation", () => {
  /**
   ...
   * task analysis never computes on the request path. Without
   * ensureTaskAnalysisRow every never-mutated task stayed "pending" forever.
  ...
  test("a task that has never had a row still gets one computed", async () => {
    const t = await seedTask();
    expect(await readTaskAnalysisRow(env.DB, t.taskIdNum)).toBeNull();

    await revalidateTaskAnalysis(env, t.taskIdNum);
  ...
  test("a cold GET schedules work that actually converges to a report", async () => {
    ...
    for (let i = 0; i < 30; i++) {
      const row = await readTaskAnalysisRow(env.DB, t.taskIdNum);
  ...
  test("a stale writer cannot regress a newer report", async () => {
    const t = await seedTask();
    await computeAndStoreTaskAnalysis(env, t.taskIdNum, 5);
    const newer = await getRow(t.taskIdNum);

    // A slow writer that lost the lease race finishing with an older rev.
    await computeAndStoreTaskAnalysis(env, t.taskIdNum, 2);
  ...
  test("a metric-version roll makes every stored row stale with no migration", async () => {
    ...
    await env.DB.prepare(
      `UPDATE task_analysis SET analysis_version = analysis_version + 1
       WHERE task_id = ?`
    )
      .bind(t.taskIdNum)
      .run();
...
describe("task analysis invalidation is shared with scores", () => {
  /**
   ...
   */
  test("an IGC upload through the real route marks the task analysis stale", async () => {
    const t = await seedTask();
    await computeAndStoreTaskAnalysis(env, t.taskIdNum, 0);
    const before = await getRow(t.taskIdNum);
...
describe("comp-level aggregate", () => {
  test("a stale-but-served task is NOT counted pending, and a stale refusal IS", async () => {
    ...
    await env.DB.prepare(
      ...
      .bind(secondId)
      .run();
    const refusal = await computeAndStoreTaskAnalysis(env, secondId, 0);
    expect(refusal.error).toMatch(/needs tracks/i);

    // Mark BOTH stale (as any scoring-input mutation would).
    await env.DB.prepare(
      `UPDATE task_analysis SET inputs_rev = inputs_rev + 1`
    ).run();
