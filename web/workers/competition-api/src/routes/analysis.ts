/**
 * Task analysis endpoints — the behavioural metrics (climbing, gliding,
 ...
 * comp, admin-only for a hidden `test` comp. See canViewAnalysis() below.
 ...
 *
 * Stale-first like scores (see task-analysis-store.ts), with one deliberate
...
import {
  bumpTaskAnalysisInputs,
  decodeTaskAnalysisRow,
  fieldRowHasResult,
  isFieldRowStale,
  readTaskAnalysisRow,
  readTaskAnalysisRowsForComp,
  scheduleTaskAnalysisRevalidation,
  type StoredTaskAnalysis,
  type TaskAnalysisRow,
} from "../task-analysis-store";

/**
 * Who may read task analysis. Public for a normal comp; a hidden `test` comp
 ...
 */
async function canViewAnalysis(
  c: Context<AuthedEnv>,
  compId: number,
  isTest: boolean
): Promise<boolean> {
...
/**
 * Cache-Control for task-analysis responses — matches the score route.
...
export const analysisRoutes = new Hono<AuthedEnv>()
  ...
  .get(
    "/api/comp/:comp_id/task/:task_id/analysis",
    optionalAuth,
    sqidsMiddleware,
    async (c) => {
      ...
      // isn't a signal.
      if (!(await canViewAnalysis(c, compId, !!comp.test)))
      ...
      const row = await readTaskAnalysisRow(c.env.DB, taskId);
      const stale = row ? isFieldRowStale(row) : true;

      if (row && fieldRowHasResult(row)) {
        if (stale) scheduleTaskAnalysisRevalidation(c, [taskId]);
        // `fa:` prefix because scores and task analysis share the same
        ...
        const body = await decodeTaskAnalysisRow(row);
        if (body) {
          return c.json(
        ...
        await bumpTaskAnalysisInputs(c.env.DB, [taskId]);
      }
      ...
      scheduleTaskAnalysisRevalidation(c, [taskId]);
      return c.json(
        {
          task_id: encodeId(c.env.SQIDS_ALPHABET, taskId),
          comp_id: encodeId(c.env.SQIDS_ALPHABET, compId),
  ...
  // ── GET /api/comp/:comp_id/analysis ── Comp aggregate (public)
  //
  ...
  .get(
    ...
    async (c) => {
      ...
      const rows = await readTaskAnalysisRowsForComp(c.env.DB, compId);
      ...
      const included: { row: TaskAnalysisRow; taskIdNum: number; index: number }[] = [];
      ...
      if (scheduleIds.length > 0) {
        scheduleTaskAnalysisRevalidation(c, scheduleIds);
      }
      ...
      for (const { row, taskIdNum, index } of included) {
        const report: StoredTaskAnalysis | null = await decodeTaskAnalysisRow(row);
        ...
        if (!report) {
          ...
          pendingCount++;
          await bumpTaskAnalysisInputs(c.env.DB, [taskIdNum]);
          scheduleTaskAnalysisRevalidation(c, [taskIdNum]);
          continue;
        }
  ...
  // ── POST /api/comp/:comp_id/task/:task_id/analysis/refresh ──
  //
  ...
  .post(
    "/api/comp/:comp_id/task/:task_id/analysis/refresh",
    requireAuth,
    sqidsMiddleware,
    requireCompAdmin,
    async (c) => {
      ...
      await audit(c.env.DB, c.var.user, compId, {
        ...
        description: `Triggered a manual task-analysis recompute for task "${task.name}"`,
      });
