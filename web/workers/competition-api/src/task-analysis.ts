...
/**
 * How many tracks one task analysis may hold in memory at once.
 *
 * Unlike scoring, task analysis needs EVERY pilot's raw fixes simultaneously
 ...
 */
export const MAX_TASK_ANALYSIS_TRACKS = 80;

/** A task shape task analysis cannot describe — surfaced as a 422, not a 500. */
export class TaskAnalysisUnsupported extends Error {
  constructor(message: string) {
    super(message);
    this.name = "TaskAnalysisUnsupported";
  }
}

/** One pilot class's task-analysis report plus what the comp aggregate needs. */
export interface TaskAnalysisClass {
  pilot_class: string;
...
/** The stored/served task-analysis blob for one task. */
export interface TaskAnalysisResponse {
  task_id: string;
  comp_id: string;
  task_date: string;
  classes: TaskAnalysisClass[];
}

/**
 * Compute the behavioural task analysis for one task, per pilot class.
 *
 ...
 */
export async function computeTaskAnalysis(
  taskId: number,
  db: D1Database,
  r2: R2Bucket,
  alphabet: string
  ...
  if (cfg.scoringFormat === "open_distance") {
    // Task analysis is built around a turnpoint task: legs, speed sections,
    // start gates, ESS. An open-distance task has a single take-off cylinder
    // and none of that structure.
    throw new TaskAnalysisUnsupported(
      "Task analysis is not available for open-distance tasks"
    );
  }
  if (cfg.scoredTracks.length === 0) {
    throw new TaskAnalysisUnsupported(
      "Task analysis needs tracks — none have been submitted for this task yet"
    );
  }
  if (cfg.scoredTracks.length > MAX_TASK_ANALYSIS_TRACKS) {
    throw new TaskAnalysisUnsupported(
      `Task analysis is limited to ${MAX_TASK_ANALYSIS_TRACKS} tracks per task ` +
  ...
  const classes: TaskAnalysisClass[] = [];
  ...
  for (const pilotClass of cfg.scoredClasses) {
    ...
    try {
      ...
      console.error("task analysis failed for class", pilotClass, err);
      excluded.push({
