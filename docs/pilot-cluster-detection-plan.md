...
> for, `detectGaggles` now also powers the engine's task analysis — the
> gaggle metrics in `web/engine/src/analysis/` feed it frames from the
> engine's shared resampler (`analysis/resample.ts`). As shipped,
