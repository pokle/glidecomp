...
This section specifies the SQL tables stored in the existing `taskscore-auth` Cloudflare D1 database (shared with Better Auth). Using a single database enables foreign key constraints against the `user` table and avoids cross-database complexity. Competition tables have their own migration files in the shared `web/db/migrations/` directory (starting with `0001_comp_init.sql`), separate from Better Auth's `0001_auth_init.sql`.
...
- user: Existing entry in the user table (see `web/db/migrations/0001_auth_init.sql`)
...
- [x] Create competition migration file (`web/db/migrations/0001_comp_init.sql`) with all tables (`pilot`, `comp`, `comp_admin`, `comp_pilot`, `task`, `task_class`, `task_track`)
