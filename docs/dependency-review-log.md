...
- **This cycle almost carried forward two stale "Packages Not Upgraded" rows: `leaflet` and `@pokle/basecoat`.** Neither exists in any `package.json`, `bun.lock`, or source file in this repo any more — confirmed via grep across the whole tree, not just the root. `@pokle/basecoat` was a companion to the shadcn/Base UI kit, gone since the RAC migration finished 2026-07-27 (see CLAUDE.md); it had already been wrongly carried into the 2026-08-02 and 2026-08-09 entries too, after that migration. `leaflet` genuinely was a real dependency once — the 2026-05-03 entry documents an actual CVE-2025-69993 XSS fix applied to a real `leaflet-provider.ts` file — but that file and every other trace of leaflet are gone from the current tree (replaced by `mapbox-provider.ts`), and no entry between 2026-05-03 and today ever logged the removal or migration. Its "Not Upgraded" row was evidently just never re-verified after that point and kept getting copied forward unchanged for months. Both rows removed here; a companion cleanup PR fixes the 2026-08-09 and 2026-08-02 entries the same way (the only two I could confirm postdate the actual removals with confidence — earlier entries are left alone since I can't pin down exactly when between 2026-05-03 and 2026-07-26 leaflet actually disappeared, and rewriting those without evidence would just trade one guess for another). **Don't trust a "Packages Not Upgraded" row just because a prior entry has it — cross-check every row against the current `package.json`/`bun.lock`/source tree before copying it into a new entry, the same way you'd verify any other claim in this log.**
...
- **[Retroactive correction, 2026-08-24]** This entry's "Packages Not Upgraded" table originally listed `leaflet` and `@pokle/basecoat` as still-deferred current dependencies. Neither was true even at the time: `@pokle/basecoat` had already been removed by the 2026-07-27 RAC migration, and `leaflet` had no trace left anywhere in the tree (last confirmed real in the 2026-05-03 entry, which fixed an actual `leaflet-provider.ts`). Both rows were copy-paste carryovers nobody re-verified; removed here as part of a wider stale-dependency cleanup — see the 2026-08-23 entry's Lessons for the full story.
