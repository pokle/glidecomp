...
| 2026-07-29 | Claude   | Re-review of 22 commits (base `7486d12` = prior round's reviewed HEAD → HEAD `2b2ac90`): **CIVL world pilot rankings import (#512)** — new `pilot_ranking` table (migration 0025), GitHub Actions cron script + public `GET /api/civl-rankings.csv`; **comp-scores pivot-ready CSV download (#508)**; six **report-card** commits (validity sparklines, distance distribution, glossary, standings, track-cleaning chart, scoring-curve chart, ESS arrival order); the SEC-35 3dvis cache fix landing (`e499008`, already recorded last round); CLAUDE.md pared down to rules-only. New finding SEC-36 (Low, CSV formula injection in the new civl-rankings route) | **SEC-36 fixed inline (this PR)** + regression test; **SEC-28 (open since 2026-07-06) also fixed inline this PR** + round-trip regression test, closing the last carrying CSV-injection Low; gap #12 (frontend vitest suite excluded from `test:all`) confirmed closed by #511; `bun audit` unchanged (5 residual, SEC-34); all prior fixes intact; no Critical/High/Medium |
...
- **Frontend DOM-sink + report-card data-leak sweep (subagent).** No `dangerouslySetInnerHTML`/raw `innerHTML` in any file touched this window; new report-card data (arrival order, sparkline series, standings) is fetched through the existing `/api/comp/:id/scores` endpoint (reusing its authz/visibility rules, not a new surface) and rendered via ordinary React JSX text/`aria-label` props.
