...
- **Dependency work.** `bun audit` at the prior round's HEAD-equivalent state (after `bun install` against the current lockfile) reported **16 vulnerabilities** (6 high, 9 moderate, 1 low) — up from the 5 residual majors carried since 2026-07-26. Investigated the jump: `web/frontend/package.json` still listed `shadcn` (`^4.16.1`, in `dependencies`) and `@base-ui/react` (`^1.1.0`) as runtime dependencies, both **confirmed unused** (`grep` for `@base-ui/react` imports across `web/frontend/src` returns zero hits; no `components.json`; CLAUDE.md has stated the shadcn/Base UI kit is gone since 2026-07-27) — dead weight left over from the RAC migration that pulled in `ip-address`, `brace-expansion` and `fast-uri` transitively (5 of the 16 advisories). Removed both **this PR**; `bun audit` dropped to 11. The remaining set included a **new, production-relevant** advisory: `hono <4.12.34` — ReDoS in the CORS middleware via `Access-Control-Request-Headers` (GHSA-8j4g-w8fx-2239) — moderate, and unlike the other residual advisories this one runs in both `auth-api` and `competition-api`'s request path every time a browser preflights (both workers use `cors()` from this package, confirmed by `grep`). `^4.12.33`'s range already permitted the fix; bumped explicitly to `^4.13.0` across all four workspaces (`package.json`, `web/frontend`, `web/workers/auth-api`, `web/workers/competition-api`) and reinstalled — `bun audit` dropped to **10**. Residual 10 (3 high, 6 moderate, 1 low): `astro` ×3 (moderate/low, dev/build-time — SEC-34), `sharp` ×1 (high, dev/build-time via astro/wrangler/vitest-pool-workers' own sharp requirement, despite the frontend's own direct dependency already being on `^0.35.3` — SEC-34), `react-router` ×1 (high, RSC-mode-only — SEC-34, GlideComp does not use RSC/framework mode), `undici` ×5 (1 high, 4 moderate — new to the visible count this round, pulled in by `wrangler`/`@cloudflare/vitest-pool-workers`, both dev-only tooling never bundled into a deployed Worker or the frontend). `bun run typecheck:all` and `bun run test:all` both run clean after the dependency changes: 1339 engine/scripts + 622 frontend + 657 competition-api + 97 auth-api tests, all passing.
...
| SEC-34 | Dirty `bun audit` — dev/build-time advisories                     | **Open — residual composition changed, still 10** | `astro` ×3, `sharp` ×1, `react-router` ×1 (RSC-mode-only) carried unchanged; `undici` ×5 newly visible this round (via `wrangler`/`@cloudflare/vitest-pool-workers`, dev-only). This round's *own* new `hono` ReDoS advisory was fixed inline (not folded into SEC-34's residual). Gap #18. |
...
- **`bun audit` dependency cleanup.** Removed two confirmed-unused runtime dependencies from `web/frontend/package.json` — `shadcn` (`^4.16.1`) and `@base-ui/react` (`^1.1.0`), both dead since the RAC migration completed (2026-07-27) but never removed from the manifest — clearing 5 advisories (`ip-address`, `brace-expansion`, `fast-uri`) that were only reachable through them. Bumped `hono` from `^4.12.33` to `^4.13.0` across all four workspaces (root, `web/frontend`, `web/workers/auth-api`, `web/workers/competition-api`) to close a **new, production-relevant** moderate ReDoS in the CORS middleware (GHSA-8j4g-w8fx-2239) that both `auth-api` and `competition-api` exercise on every cross-origin preflight — unlike the rest of the residual advisories (astro/sharp/react-router/undici), which are all dev/build-time only. `bun audit`: 16 → 10. No regression test was written for the ReDoS fix itself (it's a dependency-version bump, not application code), but `bun run typecheck:all` and `bun run test:all` were both run clean afterward as the verification.
