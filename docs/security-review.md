...
| 2026-07-12 | Claude   | Re-review of 49 commits (`8d86ef4..HEAD` = `84fc5ca`): **SSR public comp pages (#289)**, manual-flight (#306) + waypoints (#308/#316) + pilot-status (#302) mutating endpoints, stale-first D1 scoring (#279), super-admin cache route, GAP start gates (#268), engine review fixes (#319–#331). New finding SEC-29 (Low, client-side GPX/KML parser quadratic blowup) | No Critical/High/Medium; SSR surface well-defended; SEC-29 documented (deferred); `bun audit` clean; all prior fixes intact |
| 2026-07-19 | Claude   | Re-review of 45 commits (`84fc5ca..HEAD` = `e5e5fb6`): **email-OTP passwordless sign-in (#346)** + auto-derived usernames (#349), **Field Analysis engine + admin-only endpoints (#379–#383)**, GAP stopped-tasks / leading-weight / deadline scoring (#361–#367), RAC UI kit + task-page conversion (#373), waypoint editor (#341–#344), `stop_announcement_time` task field + `POST /rescore` + `field-analysis/refresh` endpoints, TypeScript 7 (#368/#371), IGC-validator rewrite to `gunzipSync`. New findings SEC-30 (Medium, open redirect) + SEC-31 (Low, field-analysis memory cap) | **SEC-30 fixed inline (this PR)**; SEC-31 documented (deferred); `bun audit` clean; all prior fixes intact |
| 2026-07-28 | Claude   | Re-review of 17 commits (base `e06aa34` = prior round's doc commit → HEAD `7486d12`): **SSR-seeded current user (#492)** + `AUTH_API` binding in the Pages Function, **server-rendered query strings (#488)**, **static-page account menu (#479)**, **transient-blip auth retry (#481/#484)**, **dev-router Worker (#480)**, container preview (#466), 3dvis ETag/304 revalidation + `taskRouteSummary` comp-detail trim, `_headers` immutable-asset rules, compact task diagrams (#474/#476), RAC chrome wave 1 (#487). New finding SEC-35 (Low, 3dvis bundle `public`-cached for signed-in viewers) | **SEC-35 fixed inline (this PR)** + regression test; `bun audit` down to the 5 deferred majors (SEC-34 residual); all prior fixes intact; no Critical/High/Medium |
| 2026-07-26 | Claude   | Re-review of 49 commits (base `e84e256` = prior round's doc commit in the current lineage → HEAD `7947d59`): **Task weather (#444/#461/#462/#467)** — new `web/engine/src/weather/**` provider registry + three Open-Meteo adapters, `routes/weather.ts` (public GET + admin refresh), `weather-store.ts` (migration 0023), and organizer `weather_notes`; **field analysis made PUBLIC + SSR'd (#430)**; **track-quality withholding (#446)** + **altitude cleaning (#408/#435)**; **FTV series scoring (#426)** + `series_scoring`/`ftv_factor` comp fields; `PATCH …/quality-override` (migration 0024); canonical comp URLs + `functions/_middleware.ts` 301 (#432/#419); pilot-style clustering (#429); mapbox-gl 3.27.0. New findings SEC-32 (Medium, quadratic altitude-cleaning DoS on crafted IGC) + SEC-33 (Low, quadratic track-quality glide window) + SEC-34 (Medium, dirty `bun audit`, dev/build-time) | **SEC-34 partially fixed inline** (shell-quote + postcss override refresh, 2 highs cleared); SEC-32/33 documented (deferred — SCORING_ROOTS, needs engine-version bump); remaining 7 dep advisories deferred to `upgrade-deps`; all prior fixes intact |
| 2026-07-29 | Claude   | Re-review of 22 commits (base `7486d12` = prior round's reviewed HEAD → HEAD `2b2ac90`): **CIVL world pilot rankings import (#512)** — new `pilot_ranking` table (migration 0025), GitHub Actions cron script + public `GET /api/civl-rankings.csv`; **comp-scores pivot-ready CSV download (#508)**; six **report-card** commits (validity sparklines, distance distribution, glossary, scores, track-cleaning chart, scoring-curve chart, ESS arrival order); the SEC-35 3dvis cache fix landing (`e499008`, already recorded last round); CLAUDE.md pared down to rules-only. New finding SEC-36 (Low, CSV formula injection in the new civl-rankings route) | **SEC-36 fixed inline (this PR)** + regression test; **SEC-28 (open since 2026-07-06) also fixed inline this PR** + round-trip regression test, closing the last carrying CSV-injection Low; gap #12 (frontend vitest suite excluded from `test:all`) confirmed closed by #511; `bun audit` unchanged (5 residual, SEC-34); all prior fixes intact; no Critical/High/Medium |
| 2026-08-05 | Claude   | Re-review of 37 commits (base `2b2ac90` = prior round's reviewed HEAD → HEAD `dac06c5`): **anonymous track submission front door (#535)** — new `igc-anon.ts`/`open-comps.ts`/`registration.ts`/`submission-gate.ts`/`rate-limit.ts`, `task.submissions_closed` (migration 0028), `comp.open_registration` (migration 0027); **site search (#524)** — new `search.ts`/`search-index.ts` over a self-maintaining FTS5 index (migration 0026); **IGC Forge** test-tracklog synthesiser (`forge-igc.ts`, client-only); **SEC-32/33 engine fix landing** (`a7202f5`) with `SlidingMedian`/monotone-deque replacements + regression tests; **reconstructed thermals** (`f73f3f9`, new `thermal-shape.ts`); report-card ledger reconciliation + winner's-missing-points; onboarding widened to email sign-ups. New findings SEC-37 (Medium, `registration.ts` missing `test`-comp gate) + SEC-38 (Low, signed-in IGC upload routes missing `test`-comp gate) + SEC-39 (Medium, anonymous-submission rate-limit budget chargeable before any legitimacy check) + SEC-40 (Low, unbounded O(k²) clustering in new `thermal-shape.ts`) | SEC-32/33 **confirmed genuinely fixed** (verified fix mechanism + regression tests, not just the commit message); SEC-37/38/39/40 documented (deferred, all Medium/Low); `bun audit` cleaned from 16→10 (removed 1 genuinely-dead dep, bumped `hono` to close a production-relevant CORS ReDoS — a same-PR CI-caught mistake removed a second, still-load-bearing dep and then restored it) **this PR**; all other prior fixes intact; no Critical/High |
| 2026-08-12 | Claude   | Re-review of 53 commits (base `dac06c5` = prior round's reviewed HEAD → HEAD `e94b491`): **FAI S7F 2026 scoring migration** (single-edition, #598) + **S7F §7.1 PathFinder route optimiser** (new `web/engine/src/route-optimizer.ts`, #612) + engine "thermo-nuclear" code-quality review (#613); **worker code-quality review** (#564) extracting a new `web/workers/shared` package (CORS allowlist + rate-limit budgets, used by all three workers); CIVL-ranking roster matching + WPRS points (migrations 0029/0030) and an "official results" annotation (migration 0031, #603/#608) shown beside rescored scores; IGC sha256 dedup (migration 0032, offline reseed only); `pilot.ts` split into `pilot-profile.ts`/`pilot-analysis.ts`; MasterDetail component consolidation, thermal-rose chart, CIVL-fill dialog, cylinder-tolerance route-editor UI. Five parallel subagents (shared CORS/rate-limit package; new competition-api routes; upload/validation pipeline; engine parsers + new algorithmic code; frontend DOM-sink/CSV/href sweep) plus direct `bun audit`/typecheck/test runs. New findings SEC-41 (High, unescaped waypoint/event names into `innerHTML` across 4 analysis-page files) + SEC-42 (Medium, unvalidated `javascript:`-capable URL in the new official-results `href`) + SEC-43 (Low, route-editor CSV export missing the SEC-28/36 formula-injection guard) + SEC-44 (Medium, `PATCH /pilot/:id` omits 3 fields from the audit log) + SEC-45 (Medium, new O(fixes)×O(turnpoints²) route-optimiser search, reachable via anonymous track upload) + SEC-46 (Low, pre-existing `circle-detector.ts` same-timestamp O(n²) scan, newly ticketed) | **SEC-41/42/43/44 fixed inline (this PR)** + regression tests where the surface allows it; SEC-45/46 documented (deferred — need SEC-32/33-style oracle tests); `bun audit` improved 10→3 (weekly dep upgrade #605 already landed in-window); SEC-37/38/39 all confirmed still genuinely fixed; SEC-32/33/40 re-confirmed unchanged; no Critical |
| 2026-08-17 | Claude   | Re-review of 6 commits (base `e94b491` = prior round's reviewed HEAD → HEAD `b5a558f3`): a small window — five field-analysis presentation commits (#623/#625/#627/#628/#629: findings digest, pilot pin + picker, thermal-rose gestures, TOC labels, final-glide gate anchoring; engine side is additive `winning` metadata + one O(legs) change, FIELD_ANALYSIS_VERSION 22→24) plus the prior security-review PR (#624) itself landing. New finding SEC-47 (High, stored XSS: `setFlightInfo`'s pilot name — roster `registered_pilot_name` or IGC `HFPLT` header — reaches `innerHTML` unescaped; the ninth SEC-41-class sink, missed by the SEC-41 sweep in the very file it fixed) | **SEC-47 fixed inline (this PR)**; **SEC-46 fixed inline (this PR)** — persistent-pointer + budgeted-fallback rewrite of `computeBearingRates` with verbatim-oracle parity tests and 60k-fix bounded-time guards, closing the smaller of the two deferred engine-DoS findings; **gap #9 closed** — new `html-sinks.test.ts` pins zero `dangerouslySetInnerHTML` in `react/**` and an exact per-file HTML-sink count across `src/`, so a new sink now fails the suite at introduction time; SEC-42/43/44 fix landings verified in-window; `bun audit` unchanged (3 × `astro`, dev/build-time); SEC-45 deferred again (needs its own oracle-tested engine PR); no Critical |
