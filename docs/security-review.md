...
- **Resolution (follow-up PR):** both POST routes select `test` and call the same `hiddenFromSubmitter()` gate SEC-37 uses, immediately after the comp lookup — ahead of the close-date check, the task lookup, the identity resolution and the body read, so a refusal still costs a header parse rather than a megabyte. **One route beyond the finding was fixed with them:** `manual-flight.ts`'s three mutating routes (PUT record, DELETE supersede, POST restore) had the identical gap, and a manual flight is evidence for a task in exactly the way a tracklog is — `submission-gate.ts`'s own header already counts it as the fourth submission call site, so leaving it would have left the same hole under a different verb. Regression tests in `test/igc-routes.test.ts` (a non-admin's self upload and on-behalf upload each 404 and write NOTHING — no `task_track` row and no `comp_pilot` row; the comp's own admin and a super admin still upload; an ordinary comp is unaffected) and `test/manual-flight.test.ts` (a non-admin's PUT 404s and stores no row; the admin's does not). `docs/track-submission.md`'s "four things that surprise people" passage — the self-report that produced this finding — now states the rule that actually holds, and the who-can-do-what table carries a hidden-comp row.
- **Not covered by this fix:** `pilot-status.ts`'s PUT/DELETE (`routes/pilot-status.ts:177`, `:433`) have the same missing `test` gate. Marking a pilot absent/DNF/landed is not a submission, so it was left out of a submission-scoped fix — but it IS a scoring input (launch validity, S7F §9.1), so it is the same class of defect and wants the same one-line gate. Carried as gap #12 below.
...
12. ~~**SEC-37 / SEC-38** — the `test`-comp gate needs adding to `registration.ts` and the two `igc.ts` POST routes~~ — **CLOSED** (2026-08-06 follow-up PR): one shared `hiddenFromSubmitter()` in `submission-gate.ts`, applied to `registration/resolve`, both `igc.ts` POST routes and `manual-flight.ts`'s three mutations, with regression tests. **Residual:** `pilot-status.ts`'s PUT/DELETE still lack the gate — same class, not a submission, and a scoring input; wants the same one-line fix.
...
2. ~~**Close SEC-37/SEC-38 first**~~ — done in the 2026-08-06 follow-up PR. What is left of that thread: **`pilot-status.ts`'s PUT/DELETE**, the one hidden-comp write path the submission-scoped fix deliberately did not touch (gap #12), and a check that anything NEW reaching a comp by id calls `hiddenFromSubmitter()` (or `visibleCompsFilter()`) rather than re-deriving the rule — the fix's whole point was to make there be one.
