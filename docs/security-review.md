...
| 2026-06-01 | Claude   | Re-review (deps + engine bug-fix + auth-api tests); closed SEC-03 + SEC-13 | SEC-03 + SEC-13 fixed inline (this PR) |
...
- Used the otherwise-quiet round to close two long-standing Open Medium/Low findings, **SEC-03** (admin emails on public comp detail — Medium, open since the initial 2026-04-20 round) and **SEC-13** (service-worker share-target filename sanitisation — Low, open since 2026-05-04). See the status rows below for the resolutions.
...
No new application source code landed since the 2026-05-25 round; the only changes are dep upgrades (covered by `bun audit` — clean at HEAD), a single engine correctness fix for the takeoff-slice bug (no security implication), and a new auth-api vitest harness that adds regression tests for SEC-01 and SEC-07 on the auth worker. With no fresh attack surface, this round closes two long-standing Open findings inline: **SEC-03** (admin email PII on public comp detail — Medium, open since the initial 2026-04-20 round) by gating the `email` field on `comp_admin` membership and returning a new `current_user_is_admin` boolean for the frontend to consume; and **SEC-13** (service-worker share-target filename sanitisation — Low, open since 2026-05-04) by stripping control chars from `X-File-Name` and URL-encoding the cache key. Both fixes ship with new regression tests on the server side (2 new SEC-03 tests on `GET /api/comp/:comp_id`) and an updated consumer in `analysis/main.ts` for the SEC-13 cache-key change. No Critical or High findings this round; SEC-04 (IGC manufacturer-record check), SEC-05 (innerHTML pattern), SEC-06 (`bodyLimit` middleware), SEC-08 (rate-limit headers), and the scope-gap #8 (flip CSP from Report-Only to enforce) remain Open and small-diff candidates for future rounds.
...
| SEC-03  | Admin emails returned on public comp detail                            | ~~Open~~ **Fixed (2026-06-01, this PR)** | `GET /api/comp/:comp_id` now resolves the caller's `comp_admin` status once (combined with the existing test-comp gate), then either passes the full `{ email, name }` admin rows through (admin caller) or maps them to `{ name }`-only (non-admin / anonymous caller). A new `current_user_is_admin` boolean is included in the response so the frontend doesn't need to probe `admins[].email` to determine admin status — see `web/workers/competition-api/src/routes/comp.ts:228-261, 305-310`. Frontend (`web/frontend/src/comp-detail.ts:30-32, 184, 1454, 1505-1507, 2039-2041`) now reads `comp.current_user_is_admin` for the gate and falls back to `admin.name` alone when `email` is absent. The PATCH response is unchanged (admin-only route — emails still returned to the admin caller). Two new regression tests in `web/workers/competition-api/test/comp-routes.test.ts:204-237` cover anonymous and authenticated-non-admin callers: both assert `admins[0].email === undefined` and `current_user_is_admin === false`. The existing admin test now also asserts `current_user_is_admin === true`. |
| SEC-04  | IGC upload size/shape                                                  | **Open (sub-issue)** | Subsumed by SEC-11 helper. Manufacturer-record (`A…`) check on the decompressed first byte still not enforced; up to 2 MiB of non-IGC text can sit in R2 per registered pilot per task / per user. Auth-gated, bounded; staying Low. |
...
Because every `*.ts` under `web/workers/*/src/` and `web/frontend/src/` is byte-identical to the 2026-05-25 round (with the exception of the targeted SEC-03 / SEC-13 fixes applied in this PR), the prior round's detailed walks still hold. Spot-confirmed this round:
...
- **`optionalAuth` PII (SEC-15 class).** No new `optionalAuth` routes added; the prior systematic walk (comp/task/igc/audit/score/pilot-status/pilot/user-files) still applies. With SEC-03 closed in this PR, the comp-detail endpoint also now redacts admin emails for non-admin callers, eliminating the last documented PII leak on the public-readable surface.
- **Secrets.** `BETTER_AUTH_SECRET`, `GOOGLE_CLIENT_ID/SECRET` referenced as env only; no hard-coded keys in source or any `wrangler.toml`. API-key prefix still `glc_`.
...
4. Walk any new mutating endpoints (authn / authz / `audit()` / Zod) — none existed this round. Recent trajectory is the per-user files/preferences surface from 2026-05-18 plus the SEC-02/SEC-03 hardening from the last two rounds, so watch for shared/public-profile features or any new field on a public-readable endpoint that could re-introduce a SEC-15-class PII leak.
5. Confirm the SEC-03 fix held on a live deploy: `curl -s https://glidecomp.com/api/comp/<some-public-comp> | jq '{ admins, current_user_is_admin }'` should show admins[].email all absent and `current_user_is_admin: false`.
...
7. Fill in the `test.todo(...)` placeholders in `web/workers/auth-api/test/routes.test.ts` (tiers 2-4 — username format validation, API-key round-trip, rate-limit 429 with `Retry-After`). The rate-limit one would close SEC-08 if paired with a `Retry-After` header in the response.
8. Address scope-gap #8 (CSP enforce) on the next preview deploy.
