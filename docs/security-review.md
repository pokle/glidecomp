...
#### SEC-39 — Anonymous-submission rate-limit budgets chargeable before any legitimacy check — **Medium** — Open
...
13. **SEC-39** — reorder the anonymous-submission budget checks (or add a per-IP budget) so the rate limiter can't be used to exhaust itself ahead of any legitimacy check.
...
3. **SEC-39** needs a decision, not just a fix: reordering the budget check is easy, but whether to add a per-IP budget too is a product question (the doc's stated posture is "flag, don't reject" for low-stakes comps — worth checking with the project owner whether a per-IP budget fits that posture or over-corrects it).
