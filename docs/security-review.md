...
#### SEC-37 — `registration.ts` missing `test`-comp visibility gate — **Medium** — Open
...
#### SEC-38 — Signed-in IGC-upload routes missing `test`-comp visibility gate — **Low** — Open
...
12. **SEC-37 / SEC-38** — the `test`-comp gate needs adding to `registration.ts` and the two `igc.ts` POST routes. Small, mechanical, same pattern already in the same files.
...
2. **Close SEC-37/SEC-38 first** — both are one-line-per-route fixes against a pattern already present twice in `igc.ts` itself; there's no design question left to resolve, just the mechanical application.
