...
#### SEC-32 — Quadratic-time altitude cleaning on crafted/degenerate IGC timestamps — **Medium** — Open (documented; deferred — `SCORING_ROOTS`)
...
#### SEC-33 — Quadratic-time track-quality "never airborne" glide window — **Low** — Open (documented; deferred — `SCORING_ROOTS`)
...
#### SEC-34 — Dirty `bun audit`: 11 dev/build-time advisories (2 cleared inline, 7 deferred) — **Medium** — ~~Open~~ **Partially fixed (2026-07-26, this PR)**

> **Resolution (partial):** refreshed two lapsed transitive overrides — `shell-quote ^1.8.4 → ^1.10.0` (a new DoS advisory GHSA-395f covers `<=1.8.4`) and `postcss ^8.5.13 → ^8.5.18` (GHSA-r28c covers `<=8.5.17`; now resolves 8.5.23) — in the root `package.json` `overrides`, clearing **2 of the 7 highs**. `bun install` re-resolved the lock; `bun audit` now reports **9** (was 11). The remaining 7 require **major** version bumps and are deferred to the weekly `upgrade-deps` routine (see below).
...
- **Remediation:** 2 cleared inline (above). The other 7 need major bumps that carry build-break risk and belong in the dependency routine, not an unattended security PR: `astro 6→7` (GHSA-4g3v / GHSA-f48w / GHSA-7pw4 — build-time XSS in View Transitions / hydrated islands, unused by GlideComp's zero-JS prerendered pages), `react-router 7→8` (RSC-only, unreachable), `sharp 0.34→0.35` (build-time libvips CVEs), `fast-uri 3→4` + `@hono/node-server 1→2` + `brace-expansion` (transitive under the `shadcn` dev CLI). Deferred to `upgrade-deps` (gap #18). All are build/dev-only, so the deferral carries no production exposure.
...
19. **New — SEC-32/33 engine-owner fix.** Incremental sliding-window median for `altitude-cleaning.ts` + fix-count bound for `track-quality.ts`, behind a `SCORING_ENGINE_VERSION` bump and gap-2018 parity re-validation.
