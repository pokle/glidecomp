...
| `0034_hot_path_indexes` | Indexes for the hot reads that scanned whole tables: `task(comp_id, task_date)`, `comp_pilot(comp_id)`, the unclaimed-email lookup the pilot bootstrap runs on every sign-in, `comp_admin(user_id)`, `comp(test, creation_date)`, and Better Auth's `session`/`account` by `userId`. Pure additions. See [2026-09-26-workers-performance-review.md](2026-09-26-workers-performance-review.md) §3.2 |
